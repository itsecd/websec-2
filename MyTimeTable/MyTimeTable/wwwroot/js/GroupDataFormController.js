...
$(document).ready(function () {
	...
	$(window).on('resize', function () {
		...
		if (currentWidth <= 765) { InitTimeTable(timeTable, true, new Date().getDay() - 1) }
		if (currentWidth > 765) { InitTimeTable(timeTable, false, new Date().getDay() - 1) }
		winWidth = $(this).width();
	});
});
...
function InitPage() {
	...
	groupId = searchStr.substring(0, 9);
	...
	$.ajax({
		url: `https://localhost:7175/TimeTable/GetGroupTimeTable/${groupId}&week=${week}`,
		method: 'get',
		dataType: 'html',
		success: function (data) {
			...
			if (data.success) {
				timeTable = data;
				InitTimeTable(data, $(window).width() <= 765 ? true : false, new Date().getDay() - 1);
			}
			else {
...
function InitTimeTable(resp, isMobile, day) {
	if (!day && day != 0) {
		return;
    }
	if (day == 6) {
	...
	let name = resp.inGroupName;
	$("#groupName").html(name);
	htmlString = `<tr class="weekRow">`;
	htmlString += `<td class="usually week"><a style="color: white" href="https://localhost:7175/TimeTable/Group/${groupId}&week=${parseInt(resp.inPrevWeek.substring(0, 3))}">${resp.inPrevWeek.substring(0, 3)}</a></td>`;
	htmlString += `<td colspan="5" class="usually week">${resp.inCurWeek}</td>`;
	htmlString += `<td class="usually week"><a style="color: white" href="https://localhost:7175/TimeTable/Group/${groupId}&week=${parseInt(resp.inNextWeek.substring(0, 3))}">${resp.inNextWeek.substring(0, 3)}</a></td>`;
	htmlString += `</tr>`;
	if (!isMobile) {
		...
		if (prevDay || prevDay == 0) {
			htmlString += `<td class="usually day" id="prev${prevDay}"><input type="image" class="icon" src="/images/prevArrow.png" onclick="InitTimeTable(timeTable, true, ${prevDay})"/></td>`;
		}
		else {
		...
		if (nextDay) {
			htmlString += `<td class="usually day" id="next${nextDay}"><input type="image" class="icon" src="/images/nextArrow.png" onclick="InitTimeTable(timeTable, true, ${nextDay})"/></td>`;
		}
		else {
			htmlString += `<td class="usually day"></td>`;
        }
...
function fillCell(subject, isMobile, time) {
	...
	if (subject == null || (subject != null && subject[0] == null)) {
		...
		if (subject.length == 1) {
			...
			htmlString += subject[0].Teacher == null ? "" : `<a href="https://localhost:7175/TimeTable/Teacher/${subject[0].TeacherId}&week=current">${subject[0].Teacher}</a></br></br>`;
			htmlString += `</div>`
			htmlString += `</td>`;
		}
		else {
			...
			for (let k = 0; k < subject.length; k++) {
				...
				htmlString += `<a href="https://localhost:7175/TimeTable/Teacher/${subject[k].TeacherId}&week=current">${subject[k].Teacher}</a></br></br>`;
				htmlString += `</div>`
			}
			htmlString += `</td>`;
		}
